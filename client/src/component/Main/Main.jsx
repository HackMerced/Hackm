	...
	render() {
		return (
			<Switch> {/* basic switch statement */}
				<nav>
				{Routes.map((route) => { 			{/* maps over the array provided in 'Routes' using the call variable 'route' */}
					return (
					<Route
					key = {route.path} 	// Key is unique child element identifier, implemented this because we are mapping
					exact path = {route.path} 	// pulls the 'path' section from repsective object in the JSON array
